...
export const ERROR_MESSAGES = {
  ...
  profile: {
    shopNameRequired: 'وارد کردن نام فروشگاه الزامی است.',
    shopNameTooShort: (min: number) => `نام فروشگاه باید حداقل ${min} کاراکتر باشد.`,
    shopNameTooLong: (max: number) => `نام فروشگاه باید کمتر از ${max} کاراکتر باشد.`,
    handleInvalid: 'آیدی فقط می‌تواند شامل حروف انگلیسی، عدد و خط زیر (_) باشد.',
    handleTooShort: (min: number) => `آیدی باید حداقل ${min} کاراکتر باشد.`,
    handleTooLong: (max: number) => `آیدی باید کمتر از ${max} کاراکتر باشد.`,
    bioTooLong: (max: number) => `معرفی فروشگاه باید کمتر از ${max} کاراکتر باشد.`,
    addressRequiredWhenVisible: 'برای نمایش آدرس در پروفایل، ابتدا نشانی فروشگاه را وارد کنید.',
    addressTooLong: (max: number) => `نشانی فروشگاه باید کمتر از ${max} کاراکتر باشد.`,
    phoneRequired: 'وارد کردن شماره تماس فروشگاه الزامی است.',
    phoneInvalid: 'شماره تماس وارد شده صحیح نیست. نمونه صحیح: 09171234567',
    avatarSizeLimit: 'حجم تصویر پروفایل باید کمتر از ۵ مگابایت باشد.',
    avatarFormatLimit: 'فرمت تصویر پروفایل نامعتبر است. فقط عکس‌های JPG، PNG و WEBP مجاز هستند.',
    avatarUnreadable: 'تصویر انتخاب شده قابل خواندن نیست. لطفا عکس دیگری انتخاب کنید.',
    updateFailed: 'ذخیره تغییرات پروفایل انجام نشد. لطفا دوباره تلاش کنید.',
    loadFailed: 'دریافت اطلاعات پروفایل با خطا روبرو شد. صفحه را رفرش کنید.',
  },
  posts: {
