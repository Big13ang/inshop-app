...
Material the agent refers to on demand — definitions, facts, parameters, examples, conditional instructions. When a skill has steps, reference in `SKILL.md` that is long or complex buries them, making it a coin-flip whether the agent attends to the sequence of execution. The cure is **progressive disclosure**: move it down the ladder, either out of `SKILL.md` into a disclosed file or behind a pointer to external reference.

_Avoid_: documentation, instructions, specs

### Progressive Disclosure

Moving content down the **information hierarchy** — out of `SKILL.md` into a linked file, or out of the skill system entirely — so the top stays clean and readable. The key tool for keeping a skill focused. Some skills have **branches** — different runs taking different paths through the steps — which is the cleanest test for progressive disclosure: inline what every branch needs, and push behind a pointer what only some branches reach.

_Avoid_: nesting, separation of concerns

### Branch

A distinct path through the skill's **steps** or execution flow. If a skill has branches, it has more than one way to run. A key test for **progressive disclosure**: if a reference block is only needed on one branch, push it out of `SKILL.md` so other runs don't carry its context weight.

_Avoid_: mode, variant, workflow

### Co-Location

Keeping related items together — definition, rules, and examples under one heading rather than scattered. When the agent reads one part, its attention automatically spans the neighbours.

_Avoid_: proximity, grouping

## Steering

Shaping the agent's behaviour at runtime.

### Leading Word

A compact, evocative concept already rich in the model's pretraining that anchors a whole pattern of behaviour in the fewest tokens (e.g. _lesson_, _fog of war_, _tracer bullets_).
